class BattlesController < ApplicationController

  def new
    @battle = Battle.new
  end

  def index
    @question = Question.create
    raise


  end
# call the questions controller to generate the AI question + answers
# breaks apart the Qs and As and puts them on the page.
# if Monster health is above 0 call the controller to generate new questions.
# if Character health is blow 0 character flees
# calculate character and monster health based on the battle results
# depending on the results of the battle delets the monster from the map
end
